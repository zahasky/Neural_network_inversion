...
class Encoder(nn.Module):
    def __init__(self, inchannels=1, outchannels=1, filters=48, num_res_blocks=1):
        ...
        self.conv2 = nn.Conv3d(filters, outchannels, 3, 1, 1, bias=False)
        # output size, outchannels x 5 x 5 x 10

    def forward(self, img):
        # img: inchannels x 20 x 20 x 40
        out1 = self.conv1(img)         # filters x 20 x 20 x 40
        out2 = self.pool1(out1)       # filters x 10 x 10 x 20
        out3 = self.res_blocks(out2)   # filters x 10 x 10 x 20
        out4 = self.trans1(out3)        # filters x 5 x 5 x 10
        out5 = self.conv2(out4)        # outchannels x 5 x 5 x 10
...
class Decoder(nn.Module):
    def __init__(self, inchannels=1, outchannels=1, filters=48, num_res_blocks=1, num_upsample=2):
        ...
        #self.conv2 = nn.Conv3d(filters, outchannels, kernel_size=3, stride=1, padding=1, bias=False)
    def forward(self, z):
        ...
        out5 =  self.transup2(out4)     # filters x 20 x 20 x 40
        #img = self.conv2(out5)         # outchannels x 20 x 20 x 40
