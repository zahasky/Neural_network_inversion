...
class InputDataset(Dataset):
    ...
    def __init__(self, transform=None):
        ...
        index_1 = [x for x in range(500,20000,500)]
        invalid = False
...
train_dataloader = DataLoader(dataset=train_dataset, batch_size=90, shuffle=True, num_workers=2)
validation_dataloader = DataLoader(dataset=validation_dataset, batch_size=50, shuffle=True, num_workers=2)
...
# ----------
    for i, (imgs_inp) in enumerate(train_dataloader):
        ...
        g_loss_mse = pixelwise_loss(fake_target, true_target)

        KL_loss = 0.15*g_loss_k_row + 0.15*g_loss_k_col + 0.15*g_loss_k_lay
        g_loss = 0.55*g_loss_mse + KL_loss + 0.0001*g_loss_adv
...
# -----------
    for i, (imgs_v) in enumerate(validation_dataloader):
        ...
        with torch.no_grad():
            ...
            g_loss_msev = pixelwise_loss(fake_targetv, true_targetv)

            KL_lossv = 0.15*g_loss_k_rowv + 0.15*g_loss_k_colv + 0.15*g_loss_k_layv
            g_lossv = 0.55*g_loss_msev + KL_lossv + 0.0001*g_loss_advv
    ...
    print(
        ...
        % (epoch, nepochs, i+1, len(validation_dataloader), g_lossv.item(), g_loss_msev.item(), KL_lossv.item(), g_loss_advv.item())
    )
