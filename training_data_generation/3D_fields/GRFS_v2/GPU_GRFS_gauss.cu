...
// -------------------------------------------------------------------------
// Compile as:    nvcc GPU_GRFS_gauss.cu -O3 -arch=sm_61 -DSAVE
//  > arch=sm_52 (Titan X), arch=sm_61 (TitanXp), arch=sm_70 (Titan V)
// Run as:        ./a.out
// -------------------------------------------------------------------------

///////////////////////////////////
//  PROGRAM  --  GPU_GRFS_gauss  //
///////////////////////////////////
...
int main(){
  ...
  for (ih=0; ih < Nh; ih++){
    ...
    while(flag==1){
      RND(rand1);
      k = k_m*rand1;
      d = k*k*exp(-(DAT)0.5*k*k);
      RND(rand1);
      if((rand1*(DAT)2.0*exp(-(DAT)1.0)) < d){ flag = 0; }
    }
    k     = k/lf*sqrt((DAT)2.0);
    RND(rand1);
