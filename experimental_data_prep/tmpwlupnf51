...
plot_2d(por_guess[:,:,22], vox_size[0], vox_size[1], '[-]', cmap='viridis')
plt.clim(0.15, 0.25)
por_guess = por_guess[:,:,4:-5]

save_filename = 'D:\\Dropbox\\Codes\\Deep_learning\\Neural_network_inversion\\experimental_data_prep\\pet_data'  + '\\' 'Navajo_scaled_ct_coarsened.csv'
save_data = np.append(por_guess.flatten('C'), [data_size, vox_size])
np.savetxt(save_filename, save_data, delimiter=',')
