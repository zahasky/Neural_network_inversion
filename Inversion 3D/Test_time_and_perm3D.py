...
class InputDataset(Dataset):
    ...
    def __init__(self, transform=None):
        ...
        for i in index_1:
            index_2 = [y for y in range(62,63)] #indeces for every .csv file
            ...
            for j in index_2:
                ...
                try:
                    ...
                    gaussian_arr = np.random.normal(0,0.05,len(tdata_ex))
...
encoder.load_state_dict(torch.load('./encoder_epoch199.pth', map_location='cpu'))
encoder.eval()

decoder.load_state_dict(torch.load('./decoder_epoch199.pth', map_location='cpu'))
decoder.eval()
...
plt.title('Normalized Breakthrough Time Layer 10', fontsize=fs+2, **hfont)
...
ax2.tick_params(axis='both', which='major', labelsize=fs)
plt.ylabel('Distance [cm]', fontsize=fs, **hfont)
plt.title('Normalized Breakthrough Time Row 10', fontsize=fs+2, **hfont)
