...
# extract the pixels at the source location (i,j) and its surrounding eight pixels, corresponding to Figure 7 in the paper
def conc_peak(conc,id_s):
    conc_max = (th.zeros((conc.shape[0],conc.shape[1], *conc.shape[2:]), dtype=th.float)).to(device)
    conc_max[id_s] = conc[id_s]                # c(i,j)

    id_s_j1 = np.array(id_s)
    id_s_j1[[2],:] = np.array(id_s)[[2],:] + 1
    conc_max[ tuple(id_s_j1) ] = conc[ tuple(id_s_j1)] # c(i,j+1)

    id_s_j_1 = np.array(id_s)
    id_s_j_1[[2],:] = np.array(id_s)[[2],:] - 1
    conc_max[ tuple(id_s_j_1) ] = conc[ tuple(id_s_j_1)] # c(i,j-1)

    id_s_i1 = np.array(id_s)
    id_s_i1[[3],:] = np.array(id_s)[[3],:] + 1
    conc_max[ tuple(id_s_i1) ] = conc[ tuple(id_s_i1)] # c(i+1,j)
    id_s_i_1 = np.array(id_s)
    id_s_i_1[[3],:] = np.array(id_s)[[3],:] - 1
    conc_max[ tuple(id_s_i_1) ] = conc[ tuple(id_s_i_1)] # c(i-1,j)

    id_s_i_1_j_1 = np.array(id_s)
    id_s_i_1_j_1[[2,3],:] = np.array(id_s)[[2,3],:] - 1
    conc_max[ tuple(id_s_i_1_j_1) ] = conc[ tuple(id_s_i_1_j_1)] # c(i-1,j-1)
    id_s_i1_j1 = np.array(id_s)
    id_s_i1_j1[[2,3],:] = np.array(id_s)[[2,3],:] + 1
    conc_max[ tuple(id_s_i1_j1) ] = conc[ tuple(id_s_i1_j1)] # c(i+1,j+1)

    id_s_i_1_j1 = np.array(id_s)
    id_s_i_1_j1[[2],:] = np.array(id_s)[[2],:] + 1
    id_s_i_1_j1[[3],:] = np.array(id_s)[[3],:] - 1
    conc_max[ tuple(id_s_i_1_j1) ] = conc[ tuple(id_s_i_1_j1)] # c(i-1,j+1)
    id_s_i1_j_1 = np.array(id_s)
    id_s_i1_j_1[[2],:] = np.array(id_s)[[2],:] - 1
    id_s_i1_j_1[[3],:] = np.array(id_s)[[3],:] + 1
    conc_max[ tuple(id_s_i1_j_1) ] = conc[ tuple(id_s_i1_j_1)] # c(i+1,j-1)
...
for epoch in range(1, args.n_epochs + 1):
    ...
    for batch_idx, (input, target) in enumerate(train_loader):
        input, target= input.to(device), target.to(device)
        conc = target[:,[0]]  # added
        source = input[:,[1]] # added
        source = source.data.cpu().numpy()
        id_s = np.where(source>0) # The location of source
        conc_max = conc_peak(conc, id_s)

        model.zero_grad()
        output = model(input)

        conc_hat = output[:,[0]]
        conc_hat_max = conc_peak(conc_hat, id_s)

        loss_all = F.l1_loss(output, target,size_average=False)
        loss_peak = F.l1_loss(conc_hat_max, conc_max,size_average=False)
        loss = loss_all + args.w_c*loss_peak
    ...
    if epoch % args.log_interval == 0:
        ...
        print("loss_all: {}, loss_peak: {}".format(loss_all, args.w_c*loss_peak))
