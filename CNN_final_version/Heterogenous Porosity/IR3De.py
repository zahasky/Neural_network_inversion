...
from EncDec3De import Encoder, Decoder
...
parser.add_argument('--idx-train', type=int, default=19200, help='Index of the last training data')
parser.add_argument('--idx-val', type=int, default=24000, help='Index of the last validation data')
...
for epoch in range(1,args.num_epochs+1):
    ...
    for i, (imgs_inp) in enumerate(train_dataloader):
        ...
        inp = torch.cat((true_inp_P,prsity),1)

        # Encoding-Decoding model
        lat_target = encoder(inp)
    ...
    for i, (imgs_v) in enumerate(validation_dataloader):
        for j, (imagev) in enumerate(imgs_v):
            ...
            if j == 0:
                ...
                prsityv = Variable(imagev.type(Tensor))

        with torch.no_grad():
            inpv = torch.cat((true_inp_Pv,prsityv),1)
            # Encoding-Decoding model
            lat_targetv = encoder(inpv)
