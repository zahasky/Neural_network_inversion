...
The network training was a supervised process. The inputs were 3-D image tensors containing the arrival time difference maps () and the permeability maps of the corresponding synthetic geologic core were the target data (). The predicted permeability maps (<img src="./Figures/xhat.gif"></img>) were compared with the target synthetic permeability maps through loss functions. Training accuracy was evaluated on the test set by individually comparing each synthetic permeability map with the network predicted permeability map or by comparing the experimental arrival time difference maps and the modeled arrival time difference maps () based on the network permeability map prediction.
<p align="center">
